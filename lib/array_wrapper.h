/* 
  Original Author: Tanner Sansbury
  See source: https://stackoverflow.com/questions/18882089/wrapping-arrays-in-boost-python

  Modification: Shuangyi Tong <s9tong@edu.uwaterloo.ca>
  Oct 12, 2018
*/
...
public:
  ...
  set_slice(
    Container& container, index_type from,
    index_type to, Iterator first, __attribute__((unused))Iterator last
  )
  {
    // If range is invalid, return early.
    if (from > to) return;
...
template <typename T>
boost::python::object wrap_array(T array)
{ 
  return array_wrapper::make_array_aux(array);
}